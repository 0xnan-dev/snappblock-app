...
type TakePhotoRouteProp = RouteProp<any, 'TakePhoto'>;

type TakePhotoProps = {
  route: TakePhotoRouteProp;
  navigation: any;
};
...
const TakePhoto = ({route, navigation}: TakePhotoProps) => {
  ...
  console.log(
    'sendToDirect, username, navigation',
    sendToDirect,
    username,
    navigation,
  );
  ...
  useEffect(() => {
    if (focused) {
      CameraRoll.getPhotos({assetType: 'Photos', first: 1000}).then(
        (result: {edges: any}) => {
          const photos = result.edges;
          const groupList: any = Array.from(
            new Set(
              photos.map(
                (photo: {node: {group_name: any}}) => photo.node.group_name,
              ),
            ),
          );
          setGroups(groupList);
  ...
  const _onSelectImage = (index: number) => {
    // console.log('index', index, multiple);
    if (multiple) {
      ...
      images.push({
        ...
        base64: '',
        extension:
          photos[index]?.node?.image?.filename?.split('.').pop() || 'jpg',
      });
      navigation.navigate('StoryProcessor', {
        images,
        sendToDirect,
