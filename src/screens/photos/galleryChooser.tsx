...
type GalleryChooserRouteProp = RouteProp<any, 'GalleryChooser'>;

type GalleryChooserNavigationProp = StackNavigationProp<any, 'GalleryChooser'>;
...
const GalleryChooser = ({navigation, route}: GalleryChooserProps) => {
  //   const user: any = {}; //store.getState().user.user.userInfo;
  ...
  useEffect(() => {
    if (selectedGroupIndex > -1) {
      CameraRoll.getPhotos({
        ...
        const photoResults = result.edges;
        setPhotos(photoResults);
        if (photoResults.length > 0 && selectedIndex < 0) {
          setSelectedIndex(0);
        }
      });
    }
    return () => {};
  }, [selectedGroupIndex, page, groups, selectedIndex]);
  ...
  useEffect(() => {
    if (selectedIndex > -1) {
      ...
      if (position > -1 && selectedPhotoSpecs[position] && multiple) {
        ...
        console.log('ref.current.currentPhoto', ref.current.currentPhoto);
      }
    }
    return () => {};
  }, [
  ...
  const _onDone = async () => {
    ...
    if (isChooseProfilePhoto) {
      ...
      const extension = img.image
        .filename!.split('.')
        .pop()
        ?.toLocaleLowerCase();
      console.log('uri, extension', uri, extension);
      setUploading(false);
      ...
      if (step === 1) {
        ...
        console.log('step', step, name, caption, address);
        ...
        if (multiple) {
          ...
          for (let i = 0; i < selectedPhotos.length; i++) {
            const photo: any = photos[selectedPhotos[i]];
            const imageUrl: string | undefined = await _addToIpfs(photo);
          ...
          const imageUrl: string | undefined = await _addToIpfs(
            photos[selectedIndex],
          );
          console.log('imageUrl', imageUrl);

          if (imageUrl) {
            await _signIscn([imageUrl]);
          }
        }
  ...
  const _addToIpfs = async (photo: any): Promise<string | undefined> => {
    const {uri, filename} = photo.node.image;
    const path: string = filename.replace(/HEIC/gi, 'jpg');
    console.log('uri, filename, path', uri, filename, path);
  ...
  const _signIscn = async (imageUrls: string[]) => {
    try {
      const payload = {
        name,
        caption,
        ...
        contentFingerprints: [],
        walletAddress: 'cosmos:12zu5qe7mdkh45e3qhq768tpzwd90q8rjgp88cu',
        stakeholders: [
          {
            entity: {
  ...
  const _onAddressChange = (address: any) => {
    setAddress({...address});
  };
      ...
      {step === 2 && (
        <ScrollView
          ...
          <View
            ...
            <TouchableOpacity
              onPress={() => {
                navigation.navigate('LocationChooser', {
                  address: {...address},
