...
type StoryProcessorRouteProp = RouteProp<any, 'StoryProcessor'>;
...
const StoryProcessor = ({ route }: StoryProcessorProps) => {
  ...
  const _onNext = async () => {
    ...
    for (let i = 0; i < ref.current.processImages.length; i++) {
      const image = ref.current.processImages[i];
      console.log('image', image);
      const imageUrl: string | undefined = await _addToIpfs(image.uri);
      if (imageUrl) {
